...
export function AppliedShiftCalendar({
  ...
  shiftEndTime,
  publishedDates = [],
  approvedSlotsMap: initialSlotsMap,
  userEmail,
  shiftId,
}: AppliedShiftCalendarProps) {
  ...
  const displayedDays = useMemo(() => {
    ...
    const fallbackStart = shiftStartTime?.slice(0, 5) || startCandidates[0] || '09:00';
  ...
  const [currentMonth, setCurrentMonth] = useState(() => {
    if (displayedDays.length > 0) {
      return new Date(displayedDays[0].date + 'T00:00:00');
    }
    if (startDate) {
  ...
  const displayedDaysMap = new Map<string, AppliedDay>();
  displayedDays.forEach((day) => {
    displayedDaysMap.set(day.date, day);
  });

  const countLabel = isResultsPublished ? '採用された日数: ' : '勤務不可として提出した日数: ';
  ...
  const handleDateClick = (date: Date) => {
    ...
    if (displayedDaysMap.has(dateStr)) {
  ...
  const selectedDay = selectedDate ? displayedDaysMap.get(selectedDate) : null;
  const isUnavailableMode = !isResultsPublished;

  return (
    <div className="space-y-4">
      <div className="text-center p-3 bg-blue-50 rounded-lg border border-blue-200">
        <span className="text-sm text-blue-700">{countLabel}</span>
        <span className="text-lg font-bold text-blue-600">{displayedDays.length}</span>
        ...
        <CardContent className="p-4">
          ...
          <div className="grid grid-cols-7 gap-1">
            {grid.map((date, i) => {
              ...
              const day = String(date.getDate()).padStart(2, '0');
              const dateStr = `${year}-${month}-${day}`;
              const displayedDay = displayedDaysMap.get(dateStr);
              const isDisplayed = !!displayedDay;
              const inRange = isInShiftRange(dateStr);
              const dayOfWeek = date.getDay();
              const isPublished =
                publishedDates === null ||
                (Array.isArray(publishedDates) && publishedDates.includes(dateStr));
              const isApproved = isPublished && displayedDay?.status === 'approved';
              const isDirectApproved = isPublished && displayedDay?.status === 'direct_approved';

              const bgClass = !inRange
                ? 'bg-gray-50 text-gray-300 border-gray-100 cursor-not-allowed'
                : isDisplayed
                  ? isUnavailableMode
                    ? 'bg-red-500 text-white border-red-600 hover:bg-red-600 shadow-md cursor-pointer'
                    : isDirectApproved
              ...
              const barClass = isUnavailableMode
                ? 'bg-red-700'
                : isDirectApproved
                  ? 'bg-orange-600'
                  : isApproved
              ...
              return (
                <button
                  key={dateStr}
                  onClick={() => isDisplayed && handleDateClick(date)}
                  disabled={!isDisplayed}
                  className={`aspect-square rounded-lg border-2 text-sm font-medium transition-all duration-200 relative ${bgClass}
                    ${dayOfWeek === 0 && !isDisplayed && inRange ? 'text-red-300' : ''}
                    ${dayOfWeek === 6 && !isDisplayed && inRange ? 'text-blue-300' : ''}
                  `}
                >
                  <div className="flex flex-col items-center justify-center h-full">
                    <span>{date.getDate()}</span>
                    {isDisplayed && (isUnavailableMode ? <Clock className="h-3 w-3 mt-0.5" /> : <Check className="h-3 w-3 mt-0.5" />)}
                  </div>
                  {isDisplayed && displayedDay && (() => {
                    ...
                    const timeText = kvSlots && kvSlots.length > 0
                      ...
                      : (isResultsPublished ? '' : `${displayedDay.start_time.slice(0, 5)}-${displayedDay.end_time.slice(0, 5)}`);
          ...
          <div className="mt-4 pt-4 border-t border-gray-200">
            <div className="flex gap-4 text-xs text-gray-600 justify-center flex-wrap">
              ...
              <div className="flex items-center gap-2">
                ...
                <span>{isResultsPublished ? '未応募' : '勤務可能'}</span>
              </div>
            </div>
          </div>
        </CardContent>
        ...
        <DialogContent>
          ...
          {selectedDay && (
            <div className="py-4 space-y-3">
              {isResultsPublished && selectedDay.status && (
                <div>
              ...
              <div className={`flex items-center gap-3 p-4 rounded-lg ${
                isResultsPublished && selectedDay.status === 'approved'
                  ? 'bg-green-50'
                  : isResultsPublished
                    ? 'bg-blue-50'
                    : 'bg-red-50'
                  ...
                  <div className={`text-sm mb-1 ${
                    isResultsPublished && selectedDay.status === 'approved'
                      ...
                      : isResultsPublished
                        ? 'text-blue-700'
                        : 'text-red-700'
                  }`}>{isResultsPublished ? '勤務時間' : '勤務不可時間'}</div>
                  {(() => {
